...
import pandas as pd
f=pd.read_csv("D:\Documents\毕业设计\code\qq44lstm427.csv",index_col="trade_date")
#lT=0
#cn=0
#mts=0
#ts=0
...
seq=sequence.pad_sequences(np.array(ss),maxlen=10,dtype='float',padding='post')
#print(seq)

#pd.DataFrame(seq).to("seq.csv")
...
# fit model
model.fit(seq, seq, epochs=300, verbose=1)
...
pd.DataFrame(yhat2).to_csv('yhat_427.csv')
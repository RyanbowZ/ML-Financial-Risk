...
qh=pd.read_csv("qqwconcat.csv",index_col="time")
hs=pd.read_csv("yweek.csv",index_col="time")
...
for i in range(len(qh)):
    ...
    train_y.append(int(hs.iloc[i]["ccadjust"]))
x_train0=train_x
y_train0=train_y
d=[]
...
for Model in range(1,12):
    ...
    if Model==1:
        ...
        clf = HistGradientBoostingClassifier(max_iter=25,l2_regularization=0.3,max_depth=30,learning_rate=0.08)
    elif Model==210:#not use
        clf = make_pipeline(StandardScaler(),SGDClassifier(max_iter=800))
    elif Model==11:
        clf = LogisticRegression(C=0.25,class_weight='balanced',max_iter=650)
    else:
        continue
...
pd.DataFrame(d).to_csv('./result/SumModelResult_week2.csv')
qh['y_real']=train_y
#qh.to_csv('y_result_compare4.csv')
